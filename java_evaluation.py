...
class JavaCodeEvaluator:
    def __init__(self, java_files_path, bpe_vocab_size=8192):
        """
        初始化评估器
        ...
        self.java_bpe_encoder = JavaBPETokenizer(vocab_size=bpe_vocab_size)
    ...
    def encode_with_bpe(self, code):
        """
        使用JavaBPEEncoder对代码进行编码
        :param code: Java代码
        :return: BPE编码后的代码
        """
        return self.java_bpe_encoder.transform(code, modality='code')
        ...
        for root, _, files in os.walk(self.java_files_path):
            for file in files:
                if file.endswith('.jsonl'):
                    ...
                    java_codes = self.read_jsonl_file(file_path)
